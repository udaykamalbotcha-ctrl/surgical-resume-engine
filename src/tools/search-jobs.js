      ...
      try {
        ...
        return {
          ...
          content: [
            {
              type: 'text',
              text: error.message || "Unknown internal error",
            },
          ],
        };
      }
...
export function searchJobsHandler(params) {
  ...
  try {
    ...
    const args = buildCommandArgs(cleanedParams);
    
    const pyScriptPath = join(__dirname, 'jobspy-cli.py');
    const workspaceRoot = join(__dirname, '..', '..');
    const pythonPath = join(workspaceRoot, 'lib', 'site-packages');

    // Execute python wrapper instead of docker
    const cmd = `"C:\\Python314\\python.exe" "${pyScriptPath}" ${args.join(' ')}`;
    ...
    // Clean result from JobSpy logging before parsing
    const jsonStart = result.indexOf('[');
    const cleanResult = jsonStart >= 0 ? result.substring(jsonStart) : '[]';
